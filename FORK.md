...
| `v0.9.1-fulcrum.22` | Two fixes for the v3 commerce header, found via visual review. (1) Grid cells did not stretch to the row height under `align-items: stretch` (the grid default) — the layout only *positioned* cell content, never resized it, so a short cell's background/border covered only its content. (2) Two-value `background-position` keywords were assigned positionally, so `top right` became x=top(0%) — pinning the image to the left. | `layout/grid.go` (stretch pass: force each cell element to its cell height when align-items:stretch, mirroring the flex stretch pass); `html/converter_helpers.go` (`parseBgPosition` swaps so vertical/horizontal keywords map to the right axis regardless of order) | (1) The v3 line-item index badge (`.order-background { background:#273e6c }`) drew its dark background only behind the number; now it fills the whole cell, matching Chrome. (2) The v3 header logo (`.logo.wbg { background-size:contain; background-position:top right }`) was pinned ~50pt left of the contact column; now its right edge aligns (folio 590 vs Chrome 588). Regression tests: `fulcrum/grid_cell_stretch_test.go`, `fulcrum/bg_position_keyword_test.go`. |
| `v0.9.1-fulcrum.23` | Content in a stretched grid cell that is a flex with border-radius did not vertically center. convertFlex wraps a flex-with-visuals in a Div; when the wrapper is height-stretched (e.g. a grid cell under align-items:stretch, fulcrum.22), only the Div grew — the flex stayed content-sized at the top, so its `align-items:center` could not center the content. | `layout/div.go` (new `fillChildHeight`: a wrapper Div with a definite/forced height stretches its lone height-settable child to its inner height); `html/converter_flex.go` (sets it on the visual-wrapper Div) | The v3 line-item index badge number now centers vertically in its filled cell (badge center within ~1pt of the row middle), matching Chrome — completing the fulcrum.22 badge fix (background fill + now vertical centering). Regression test: `fulcrum/grid_cell_stretch_test.go` (TestGridCellStretchCentersContent). |
| `v0.9.1-fulcrum.24` | Three DocGen rendering fixes found via visual review. **(1)** A run of `float:right` siblings was laid out in DOM order left-to-right, but float-right stacks from the right edge so the FIRST element ends up RIGHTMOST. **(2)** A `float:right` written AFTER inline content (the title-bar "Created By …" span) was stacked BELOW the inline text instead of rising onto its line — folio's float model only hoists a float that precedes the text it wraps. **(3)** An SVG given only a CSS `width` (no height) kept its intrinsic viewBox height instead of deriving height from the aspect ratio. | **(1)** `html/converter.go` (`floatRunToRow` reverses + right-justifies an all-`float:right` run); **(2)** `html/converter.go` (new `inlineFloatRow`: a block whose content is a leading inline run + trailing `float:right` is laid out as a flex row — inline grows, floats pushed right); **(3)** `html/converter_image.go` (`convertSVG` leaves the unspecified dimension at 0 so `SVGElement.resolveSize` derives it from the aspect ratio, falling back to intrinsic only when neither is set) | **(1)** The .NET DocGen Job traveler header floats Quantity then Production-Due right; pre-patch they rendered swapped vs Chrome (Quantity must sit rightmost). **(2)** The `.title-bar` (companyinfo spans + `float:right` "Created By …", used by PurchaseOrder/NCR/CAPA/Certification) dropped the right-hand text to a second line at the bottom of the gray band; now it sits on the first line like Chrome. **(3)** The job-label `.icon` (`.printlabel.small .icon { width:10px }`, no height) on a 108×108 viewBox rendered as a 7.5pt-wide × 108pt-TALL sliver — an invisible "missing" logo that also inflated its flex `.label-header` row to 108pt, pushing the centered "Job …" title down with a huge gap. Found via visual PDF review (cmd/pdfdiff). Regression tests: `fulcrum/float_right_order_test.go`, `fulcrum/inline_float_row_test.go`, `fulcrum/svg_aspect_ratio_test.go`. |
